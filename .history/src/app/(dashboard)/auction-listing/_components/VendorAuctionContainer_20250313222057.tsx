...
const VendorAuctionContainer = ({ show }: any) => {
  ...
  const { data, isLoading, isError, error } =
    useQuery<AuctionListingDataResponse>({
      queryKey: ["all-auctionsListing", currentPage, show],
      queryFn: () =>
        fetch(
          `${
            process.env.NEXT_PUBLIC_BACKEND_URL
          }/api/vendor/auctions?page=${currentPage}&limit=6&productType=${
            show == "industry" ? "" : show
          }`,
          {
            method: "GET",
  ...
  if (isLoading) {
    ...
    content = <TableContainer data={data?.data || []} columns={ActionColumn} />;
  }
...
import {
  AuctionListingDataResponse,
  Product,
} from "@/types/auctionData/listing";
