...
const EditProductDialog: React.FC<EditProductDialogProps> = ({
  ...
  const { data: locationsData } = useQuery({
    ...
    queryFn: () =>
      fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/store/locations/${userid}`,
        {
          headers: {
  ...
  useEffect(() => {
    if (
      ...
      const countryStates = locations
        .filter((loc: any) => loc.country === selectedCountry)
  ...
  const { data: subCategoriesData, refetch: refetchSubCategories } = useQuery({
    ...
    queryFn: () =>
      fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/subcategories/by-category/${selectedCategory}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (formData: FormData) => {
      ...
      return fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/products/edit/${product._id}`,
        {
          method: "PUT",
          headers: {
            Authorization: `Bearer ${token}`,
  ...
  useEffect(() => {
    console.log(
      "EditProductDialog mounted with open:",
      open,
      "and product:",
      product?._id
    );
          ...
          <form onSubmit={form.handleSubmit(onSubmit)}>
            <div className="flex gap-4 flex-col md:flex-row">
              <div className="w-full md:w-[58%] space-y-[16px] mt-[16px]">
                ...
                {!(
                  selectedCategory === "67d1b9ecf2ed66e4a542fe5e" ||
                  selectedCategory === "67d1ba16f2ed66e4a542fe7b"
                ) ? (
                  <div className="grid grid-cols-12 gap-4">
                    <div className="col-span-6">
                      <FormField
                    ...
                    <FormItem className="flex flex-row items-start space-x-3 space-y-0 mt-4">
                      ...
                      <div className="space-y-1 leading-none">
                        ...
                        <p className="text-xs text-muted-foreground">
                          Upload a COA document for this product
                        </p>
                      </div>
                    </FormItem>
                  )}
